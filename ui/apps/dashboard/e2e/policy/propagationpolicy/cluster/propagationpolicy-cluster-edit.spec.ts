...
// ClusterPropagationPolicy type for K8s API response
interface ClusterPropagationPolicy {
    apiVersion?: string;
    kind?: string;
    metadata?: {
        name?: string;
...
test('should edit clusterpropagationpolicy successfully', async ({ page }) => {
    // Create a test clusterpropagationpolicy directly via kubectl to set up test data
    ...
    // Setup: Create clusterpropagationpolicy using kubectl
    ...
    const targetRow = await waitForResourceInList(page, clusterPropagationPolicyName);
    ...
    const apiRequestPromise = page.waitForResponse(response => {
        return response.url().includes('_raw/clusterpropagationpolicy') && response.status() === 200;
    }, { timeout: 15000 });
    ...
    const responseData = (await apiResponse.json()) as IResponse<DeepRequired<ClusterPropagationPolicy>>;
...
kind: ${data.kind}
metadata:
  name: ${data.metadata?.name || 'test-clusterpropagationpolicy'}
spec:
  resourceSelectors:
  ...
  placement:
    ...
    if (!yamlContent || yamlContent.length === 0) {
        throw new Error(`Edit feature error: Monaco editor does not load clusterpropagationpolicy YAML content. Expected name: "${expectedName}", kind: "${expectedKind}"`);
    }
    ...
    // Cleanup: Delete the created clusterpropagationpolicy
    try {
        await deleteK8sClusterPropagationPolicy(clusterPropagationPolicyName);
    } catch (error) {
        console.warn(`Failed to cleanup clusterpropagationpolicy ${clusterPropagationPolicyName}:`, error);
    }

    // Debug
    await debugScreenshot(page, 'debug-clusterpropagationpolicy-edit.png');
});
