...
test('should edit overridepolicy successfully', async ({ page }) => {
    ...
    // Setup: Create overridepolicy using kubectl
    ...
    const targetRow = await waitForResourceInList(page, overridePolicyName);
    ...
    const apiRequestPromise = page.waitForResponse(response => {
        return response.url().includes('_raw/overridepolicy') && response.status() === 200;
    }, { timeout: 15000 });
    ...
    const responseData = (await apiResponse.json()) as IResponse<DeepRequired<OverridePolicy>>;
...
kind: ${data.kind}
metadata:
  name: ${data.metadata?.name || 'test-overridepolicy'}
