...
test('should edit job successfully', async ({ page }) => {
    ...
    const targetRow = await waitForResourceInList(page, jobName);
    ...
    const apiRequestPromise = page.waitForResponse(response => {
        ...
        return (url.includes('/api/v1/_raw/') ||
                url.includes('/api/v1/namespaces/') && (url.includes('/deployments/') || url.includes('/jobs/') || url.includes('/daemonsets/'))) &&
            response.status() === 200;
    }, { timeout: 15000 });
    ...
    const responseData = (await apiResponse.json()) as IResponse<DeepRequired<k8s.V1Job>>;
...
kind: ${data.kind}
metadata:
  name: ${data.metadata?.name || 'test-job'}
  namespace: ${data.metadata?.namespace || 'default'}
spec:
  template:
    spec:
      containers:
        - name: ${data.spec?.template?.spec?.containers?.[0]?.name || 'hello'}
          image: ${data.spec?.template?.spec?.containers?.[0]?.image || 'busybox:latest'}
          command: ${JSON.stringify(data.spec?.template?.spec?.containers?.[0]?.command || ['/bin/sh', '-c', 'echo Hello'])}
      restartPolicy: ${data.spec?.template?.spec?.restartPolicy || 'Never'}
  backoffLimit: ${data.spec?.backoffLimit || 4}`;
    ...
    if (!yamlContent || yamlContent.length === 0) {
        throw new Error(`Edit feature error: Monaco editor does not load job YAML content. Expected name: "${expectedName}", kind: "${expectedKind}"`);
    }

    // Modify YAML content (change backoffLimit from 4 to 6)
    let modifiedYaml = yamlContent.replace(/backoffLimit:\s*4/, 'backoffLimit: 6');

    // Verify modification took effect
    if (modifiedYaml === yamlContent) {
        // Try other modification methods
        const alternativeModified = yamlContent.replace(/backoffLimit:\s*\d+/, 'backoffLimit: 3');
    ...
    // Debug
    await debugScreenshot(page, 'debug-job-edit.png');
