...
export default function PanelEditor({ open, onClose, onSave, catalog, editingPanel }: PanelEditorProps) {
  ...
  const handleMetricChange = (metricName: string) => {
    ...
    if (item) {
      form.setFieldsValue({
        ...
        title: item.name.replace(/_/g, ' ').replace(/\b\w/g, (c) => c.toUpperCase()),
      });
    }
  };
  ...
  return (
    <Modal
      ...
      onOk={handleSave}
      onCancel={onClose}
      okText={editingPanel ? 'Update' : 'Add'}
      width={560}
      destroyOnClose
    >
        ...
        {selectedMetric && (
          <div style={{ marginBottom: 16, padding: '8px 12px', background: '#f5f5f5', borderRadius: 6 }}>
            ...
            <Text type="secondary" style={{ fontSize: 11 }}>
              Type: <strong>{selectedMetric.prometheusType}</strong> | Group: <strong>{selectedMetric.group}</strong>
            </Text>
          </div>
        )}
