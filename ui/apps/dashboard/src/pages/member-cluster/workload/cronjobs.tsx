  ...
  const columns: TableColumnProps<Workload>[] = [
    {
      ...
      render: (_name: string, record: Workload) => (
        <NameCell
          ...
          icon={<Icons.schedule className="w-4 h-4" />}
        />
      ),
    },
    {
  ...
  return (
    <TablePageLayout filterBar={filterBar}>
      ...
      <ViewDrawer
        ...
        icon={<Icons.schedule className="w-5 h-5 text-[var(--kd-primary-600)]" />}
        loading={viewLoading}
        detail={viewDetail}
        events={viewEvents}
        extraFields={[
      ...
      <EditDrawer
        ...
        onSave={async () => {
          ...
          try {
            ...
            if (ret.code !== 200) {
              void messageApi.error(ret.message || 'Failed to update CronJob');
              return;
            }

            setEditModalOpen(false);
            setEditContent('');
            void messageApi.success('CronJob updated successfully');
          } catch {
